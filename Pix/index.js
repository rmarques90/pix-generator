class Pix {
    constructor(pixKey, description = '', merchantName = '', merchantCity = '', amount) {
        ...
        this.txid = this._generateTxId();
        this.amount = amount.toFixed(2);

        //information provided by BACEN
    ...
    _generateTxId() {
        return this._rand(23);
    }

    _rand(length, current) {
        current = current ? current : '';
        return length ? this._rand(--length, "0123456789ABCDEFGHIJKLMNOPQRSTUVWXTZabcdefghiklmnopqrstuvwxyz".charAt(Math.floor(Math.random() * 60)) + current) : current;
    }
